...
type HandlerRequestType = {
  ...
  setNext: (url: HandlerRequestType) => HandlerRequestType;
};

const handleRequest = (link: string, cb?: Function) => {
  const url = link;
  let handler: HandlerRequestType | null = null;
  return {
    checkUrl: (expectedUrl: string) => {
      if (url == expectedUrl) {
        ...
        handler.checkUrl(expectedUrl);
      } else {
        return;
      }
    },

    setNext: (nextHandler: HandlerRequestType) => {
      handler = nextHandler;
      return handler;
    },
  };
...
const popupRequest = handleRequest("popup", () => console.log("Popup hit"));
const analyticsRequest = handleRequest("analytics", () =>
  console.log("Analytics"),
);

popupRequest.setNext(analyticsRequest);
    ...
    switch (action) {
      case "popup":
        if (url.searchParams.get("id")) {
          popupRequest.checkUrl(action);
          const shop = url.searchParams.get("id");
          if (!shop) {
            return await cors(
              request,
              new Response(
      ...
      <div class="dialog__container">
              <input id="popup-id" name="popup-id" hidden value="${result.data?.id}" />
  ...
  switch(action) {
    case "analytics":
        ...
        const result = await createAnalytics({
          ...
          eventType: "cart-to-bag"
        });

        if(!result.success) {
          console.log(result, data, "db results")
          return await cors(request, new Response(
                JSON.stringify({
                  success: false,
                  error: "An has occured"
                })
              ))
        }
